...
import { useUpdateTeacherProfile } from '../../hooks/useTeacherQueries';
import { colors, radius } from '../../theme';
import { initials } from '../../utils/format';

export function TeacherProfileScreen({ navigation }: any) {
  const { user, school, updateUser } = useAuth();
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          <View style={styles.hero}>
            ...
            <Text style={styles.role}>{school?.name || 'Teacher'}</Text>
          </View>
          <Card>
            <Text style={styles.section}>Contact details</Text>
            <Field label="First name" icon="person-outline" value={firstName} onChangeText={setFirstName} autoCapitalize="words" />
            <Field label="Last name" icon="person-outline" value={lastName} onChangeText={setLastName} autoCapitalize="words" />
